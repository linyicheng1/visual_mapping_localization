...
namespace VISUAL_MAPPING {
    ...
    void Mapping::construct_initial_map(std::vector<Frame>& frames) {
        int cnt = 0;
        Matcher matcher;
        int frame_cnt = 0;
        ...
        for (auto& frame : frames) {
            for (int i = 0; i < frame.get_features_uv().size(); i++) {
                ...
                if (depth > 0.01 && mp_ptr == nullptr) {
                    ...
                    std::shared_ptr<MapPoint> map_point = std::make_shared<MapPoint>(uv, x3D, cov, frame.id, i, cnt);
...
//                    }
        ...
        std::cout << "Constructed " << cnt << " map points" << std::endl;
